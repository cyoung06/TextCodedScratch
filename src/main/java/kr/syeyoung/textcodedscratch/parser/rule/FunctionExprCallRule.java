...
//        if (!(future.getFirst() instanceof PCloseToken || future.getFirst() instanceof OperatorNode || future.getFirst() instanceof EOSToken)) return false;
        ...
        if (next instanceof KeywordFunc) return false;
        if (next == null || next instanceof EOSToken) throw new ParsingGrammarException("Invalid location of function call :: "+id);
