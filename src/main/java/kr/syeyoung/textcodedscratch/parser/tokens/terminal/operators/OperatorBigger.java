...
public class OperatorBigger extends OperatorNode {
    public OperatorBigger() {
        super(">");
    }

    @Override
    public JSONObject operate(String parentId, String nextId, Object... terms) {
        return new ScratchBlockBuilder().op("operator_gt")
    ...
    public ConstantNode operate(ConstantNode... nodes) {
        return new BooleanToken(nodes[0].getValue(Double.class) > nodes[1].getValue(Double.class));
    }
}
